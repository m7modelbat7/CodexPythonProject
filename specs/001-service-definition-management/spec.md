...
- **FR-012**: When a save/create request fails validation, the public validation response MUST report the complete set of independently detectable violations produced according to FR-025. Each reported violation MUST identify the relevant invalid field or conflict, explain the problem in safe plain language, and, for an unsupported data type, include the supported values.
...
- **FR-014**: `GET /api/v1/service-definitions` MUST list all saved service definitions as a collection of summaries, with each definition represented exactly once. Each summary MUST contain exactly `id`, `name`, `description`, and `output_data_type` with the semantics defined by the approved OpenAPI `ServiceDefinitionSummary` schema: `id` is the definition's UUID, `name` is its canonical stored name, `description` is the stored string or JSON `null`, and `output_data_type` is one exact supported `DataType` label. A list item is not a complete service definition and MUST NOT include `inputs`; complete metadata, including ordered inputs, remains available through `GET /api/v1/service-definitions/{service_definition_id}`. Definitions MUST use the deterministic two-level comparison defined in Public API Semantics: the ASCII-case-insensitive comparison key first, followed by the exact stored canonical name; no tertiary key is used. This list introduces no pagination, search, filtering, or other list functionality.
...
- **FR-025**: Validation processing MUST collect every independently detectable violation. A check is skipped only when its prerequisite value or structure cannot be established (for example, name length and canonical duplicate checks are skipped for a non-string name, and child checks are skipped when `inputs` is not an array). One underlying problem MUST produce only one violation. Violations MUST use the deterministic ordering and translation rules defined in the Public API Semantics section. These processing rules produce the complete user-facing validation result required by FR-012.
- **FR-026**: Every public HTTP 4xx response from the composed application MUST use the documented `application/problem+json` `Problem` envelope. This includes feature/application errors such as validation `422`, duplicate-name `409`, and service-definition lookup `404`, plus reachable framework/router errors: unknown-route `404` and unsupported-method `405`. FastAPI/Starlette default JSON or plain-text error bodies and raw framework exception details MUST NOT be exposed.
...
- The list response is the closed JSON object `{ "items": [...] }`, where every element of `items` is the closed four-field summary defined in FR-014. When no definitions exist, the response is `{ "items": [] }`. Ordered input definitions are detail-only and appear in the complete response from `GET /api/v1/service-definitions/{service_definition_id}`, not in list summaries.
- List ordering starts from each canonical stored service name. The primary key maps only ASCII uppercase `A`-`Z` to ASCII lowercase `a`-`z`, leaves every other Unicode code point unchanged, and compares the result by Unicode code-point sequence, equivalently UTF-8 byte order under PostgreSQL `C` collation. The secondary key compares the exact stored canonical name using the same deterministic code-point/`C`-collation order. No Unicode normalization, locale-sensitive case folding, or tertiary key is applied. Arbitrary Unicode names remain valid, but only ASCII letters participate in case-insensitive ordering. Thus `beta`, `Alpha`, `alpha`, `Beta` list as `Alpha`, `alpha`, `Beta`, `beta`; exact case-sensitive canonical-name uniqueness remains unchanged.
...
- An unknown public route returns a generic `404` Problem with `type: /problems/not-found`, `title: Not found`, `status: 404`, `detail: No route matches the requested path`, and `errors: []`. This is distinct from the service-definition-by-ID `404` above.
- An unsupported method on a known public route returns a `405` Problem with `type: /problems/method-not-allowed`, `title: Method not allowed`, `status: 405`, `detail: The requested method is not allowed for this resource`, and `errors: []`; the safe framework-provided `Allow` header is preserved.
- At the currently approved application boundary, the only reachable framework-generated 4xx categories are request/path validation `422`, unknown-route `404`, and unsupported-method `405`. No authentication, authorization, content-negotiation, or other middleware-generated 4xx behavior is part of this feature. A shared HTTP-edge handler may safely translate a future handled framework `HTTPException` 4xx without creating a product acceptance scenario for a status that is not currently reachable.
